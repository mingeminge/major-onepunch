...
public class RedisUtil {
    ...
    public void flushToken(String token) {
        stringRedisTemplate.expire(token, TokenProperties.EXPIRE, TimeUnit.SECONDS);
        log.info("{}->已刷新过期时间", token);
    }

    public void setToken(String key, String value) {
        stringRedisTemplate.opsForValue().set(key, value, TokenProperties.EXPIRE, TimeUnit.SECONDS);
